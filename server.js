...
app.post("/send", requireAuth, async (req, res) => {
  try {
    ...
    if (!email || !password || !recipients) return res.json({ success: false, message: "Missing fields" });
    ...
    if (!checkDailyLimit(email, list.length)) return res.json({ success: false, message: "Daily limit reached" });
    ...
    console.error(err);
    return res.json({ success: false, message: "Sending failed" });
  }
});
